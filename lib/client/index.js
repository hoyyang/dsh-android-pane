...
export function PaneSidebarButton() {
    ...
    useEffect(() => {
        ...
        const verify = () => {
            ...
            if (el == null)
                return; // 按钮未挂载（无会话头部），交给生命周期巡检兜底
            if (getComputedStyle(el).borderRadius === '999px')
                return; // 样式正常
            if (++tries > 3) {
...
export function apply(ctx) {
    ...
    ctx.effect(() => {
        const heal = () => {
            ...
            if (healed)
                console.warn(`[${NS}] style tags removed/tampered externally — re-mounted (CSS ${CSS_VERSION})`);
        };
    ...
    ctx.effect(() => ctx.slots.inject('conversation.session.header.utilities', () => ctx.slots.register({ name: 'conversation.session.header.utilities', id: `${NS}:entry`, order: -1 }, PaneSidebarButton)), `${NS}: header utilities entry（order 梯队升序勿撞值：billing -2 → 本插件 -1 → plan-board -0.5 → SLog 0 → delete 1；同值会退化成「谁最后 reload 谁靠右」乒乓换位）`);
    void openCount;
}
