...
export class Adb {
    adbPath;
    defaultTimeoutMs;
    constructor(adbPath, defaultTimeoutMs = 15_000) {
    ...
    run(args, opts = {}) {
        ...
        return new Promise((resolve, reject) => {
            const child = execFile(this.adbPath, [...args], { timeout: timeoutMs, maxBuffer: 64 * 1024 * 1024, killSignal: 'SIGKILL', windowsHide: true }, (err, stdout, stderr) => {
                if (err != null && typeof err.code !== 'string') {
                    // execFile 的非退出错误（ENOENT 等）
                    reject(new AdbError(`adb 启动失败：${String(err.code ?? err.message)}`, args, null, String(stderr ?? '')));
                    return;
                }
    ...
    async runOk(args, opts = {}) {
        const r = await this.run(args, opts);
        if (r.code !== 0)
            throw new AdbError('adb 命令失败', args, r.code, r.stderr);
        return r.stdout;
    }
    ...
    async shell(serial, cmd, opts = {}) {
        return this.run(['-s', serial, 'shell', cmd], opts);
    }
    async shellOk(serial, cmd, opts = {}) {
        const r = await this.shell(serial, cmd, opts);
        if (r.code !== 0)
            throw new AdbError(`设备 shell 失败`, ['-s', serial, 'shell', cmd], r.code, r.stderr);
        return r.stdout;
    }
    /** 二进制输出命令（exec-out），返回原始 buffer。 */
    execOutBinary(args, opts = {}) {
        const timeoutMs = opts.timeoutMs ?? this.defaultTimeoutMs;
        return new Promise((resolve, reject) => {
            const child = execFile(this.adbPath, [...args], { timeout: timeoutMs, maxBuffer: 64 * 1024 * 1024, killSignal: 'SIGKILL', encoding: 'buffer', windowsHide: true }, (err, stdout, stderr) => {
                const stderrText = Buffer.isBuffer(stderr) ? stderr.toString('utf8') : String(stderr ?? '');
                if (err != null && err.code !== 0 && err.code !== undefined) {
                    reject(new AdbError('adb exec-out 失败', args, err.code ?? null, stderrText));
                    return;
                }
                resolve(stdout);
            });
