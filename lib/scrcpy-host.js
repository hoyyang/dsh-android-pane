...
export class StreamSession {
    ...
    constructor(adbPath, serial, jarPath, quality, log) {
    ...
    hasActiveViewer(windowMs = 60_000) {
    ...
    async killOrphanServers(signal) {
    ...
    async start(signal) {
        ...
        const wmSize = await run(this.adbPath, ['-s', this.serial, 'shell', 'wm size'], signal, 8_000).then((r) => r.stdout, () => '');
        const m = /(\d+)x(\d+)/.exec(wmSize);
        if (m != null)
            this.displaySize = { w: Number(m[1]), h: Number(m[2]) };
        const serverArgs = [
            '-s',
            this.serial,
            'shell',
        ...
        this.proc.stdout?.on('data', (d) => {
            ...
            if (s.includes('ERROR') || s.includes('WARN'))
                this.log(`[${this.serial}] server stdout: ${s.trim().slice(0, 300)}`);
        });
        ...
        try {
            ...
            if (codecId !== 0x68323634)
                throw new Error(`codec header 异常：期望 h264(0x68323634)，实得 0x${codecId.toString(16)}`);
            ...
            sock.on('data', (d) => this.onVideoData(d));
    ...
    async connectSocketWithDummy(port, signal) {
        const deadline = Date.now() + 15_000;
        // ① 等设备端 socket 就绪（grep exit=1 = 无匹配 = 继续轮询；仅其他错误才跳过）
        for (let i = 0; i < 25; i++) {
            if (this.proc?.exitCode != null)
                throw new Error(`scrcpy server 已退出（exit=${this.proc.exitCode}）${this.stderrTail.slice(-300)}`);
            try {
                const out = await run(this.adbPath, ['-s', this.serial, 'shell', 'cat /proc/net/unix 2>/dev/null | grep -c scrcpy_' + this.scid], signal, 3_000);
                if (out.stdout.trim() !== '' && out.stdout.trim() !== '0')
                    break;
            }
            catch (e) {
                if (!String(e).includes('exit=1'))
                    break; // 无权限/不支持 → 跳过轮询；exit=1 是 grep 无匹配，继续等
            }
        ...
        for (;;) {
            if (Date.now() > deadline)
                throw new Error(`video 握手超时（${lastErr || 'EOF'}）`);
            if (this.proc?.exitCode != null)
                throw new Error(`scrcpy server 已退出（exit=${this.proc.exitCode}）${this.stderrTail.slice(-300)}`);
            let sock = null;
            try {
                ...
                if (dummy[0] !== 0x00)
                    throw new Error(`协议头异常：期望 dummy 0x00，实得 0x${dummy[0].toString(16)}`);
                return { sock };
            }
            catch (e) {
    ...
    async connectControl(port, signal) {
        const deadline = Date.now() + 10_000;
        for (;;) {
            if (Date.now() > deadline)
                throw new Error('control 连接超时');
            if (this.proc?.exitCode != null)
                throw new Error(`scrcpy server 已退出（exit=${this.proc.exitCode}）`);
            try {
    ...
    connectWithRetry(port, signal, totalMs = 8_000) {
        ...
        return (async () => {
            for (;;) {
                try {
                    ...
                    if (Date.now() >= deadline)
                        throw new Error(`连接 scrcpy 端口 ${port} 超时（最后一次: ${String(e)}）`);
    ...
    onVideoData(d) {
        ...
        for (;;) {
            ...
            const frame = { pts, data: Buffer.from(data), key: isConfig || isKey || isKeyAccessUnit(data), config: isConfig };
            ...
            if (isConfig) {
                ...
                if (this.gop.length > 512 || this.gop.reduce((a, f) => a + f.data.length, 0) > 3_000_000)
                    this.gop = [];
            }
    ...
    waitForConfig(signal, timeoutMs = 12_000) {
    ...
    send(buf) {
        ...
        if (sock == null || sock.destroyed)
            throw new Error('control socket 未就绪（设备串流未运行）');
        sock.write(buf);
    }
    ...
    async injectSwipe(x1, y1, x2, y2, durationMs) {
    ...
    injectKey(action, keycode, metaState = 0, repeat = 0) {
    ...
    setClipboardPaste(text) {
        const payload = Buffer.from(text, 'utf8');
        if (payload.length > 1_000_000)
            throw new Error(`文本过长（${payload.length} 字节 > 1MB 上限）`);
...
export function parseH264AnnexB(data) {
...
export function splitAnnexB(data) {
    ...
    while (i < data.length - 2) {
        if (data[i] === 0 && data[i + 1] === 0 && data[i + 2] === 1) {
            ...
            if (start >= 0 && nalStart > start)
                out.push(data.subarray(start, nalStart));
            start = i + 3;
            i += 3;
        }
        else {
...
export function parseSpsSize(sps) {
    try {
...
class BitReader {
    ...
    readUe() {
        ...
        while (this.readBit() === 0) {
            zeros++;
            if (zeros > 31)
                throw new Error('SPS Exp-Golomb 异常');
        }
...
// ───────────────────────── 工具 ─────────────────────────
function run(adbPath, args, signal, timeoutMs) {
    return new Promise((resolve, reject) => {
...
function readExactly(sock, n, timeoutMs) {
    return new Promise((resolve, reject) => {
        const chunks = [];
        let have = 0;
        let timer = null;
...
export function jarPathIn(pluginRoot) {
...
//# sourceMappingURL=scrcpy-host.js.map