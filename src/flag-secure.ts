...
export function parseWindowDump(dump: string): { focusHash: string | null; windows: WinBlock[] } {
  ...
  const windows: WinBlock[] = []
  let cur: WinBlock | null = null
...
export function extractSecureState(dump: string): SecureFocus {
...
export const HINT_DISABLE_FLAG_SECURE =
  '检测到 root 框架（Magisk/LSPosed）——像素级可见需安装并启用 DisableFlagSecure 模块' +
