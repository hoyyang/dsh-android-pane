...
// B23：dsh-tools 当版类型面不再导出 JsonValue（junction 物化后实测），本地定义等价递归类型
...
function sessionIdOf(ctx: ToolCtx): string {
  ...
  if (id == null || id === '') throw new Error('无法识别当前会话（agent id 缺失），设备认领需要会话身份')
  return id
}
...
export function registerPaneTools(
  ...
  tools.register(
    defineTool({
      ...
      description:
        'List Android devices (real + emulator) with online state, model, panel claim owner and stream mode. Use before attach to pick a serial.',
      parameters: {},
      output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
      execute: async (_args, ctx: ToolCtx) => {
        void ctx
  ...
  tools.register(
    defineTool({
      ...
      description:
        'Claim an Android device for this session and open its live screen in the DSH pane (H.264 stream; auto-fallback to screenshot polling). ' +
  ...
  tools.register(
    defineTool({
      ...
      description:
        'Interact with the claimed Android device, then auto-capture a screenshot so you can verify the result. ' +
      ...
      output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
      execute: async (args, ctx: ToolCtx) => {
        return toJson(await hub.act(sessionIdOf(ctx), args as never, ctx.signal))
      },
      timeoutMs: 30_000,
    }),
  )
  ...
  tools.register(
    defineTool({
      ...
      description:
        'Dump the Android UI hierarchy (uiautomator) and return up to 100 interactive/labeled nodes with text, content-desc, class, clickability and center coordinates. ' +
  ...
  tools.register(
    defineTool({
      ...
      description:
        'Element-level UI operations backed by uiautomator authoritative coordinates — eliminates blind-coordinate tap misses entirely. ' +
      ...
      output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
      execute: async (args, ctx: ToolCtx) => {
        const { action, target, index, text, clear, serial } = args as {
          action: 'find' | 'click' | 'setText'; target?: string; index?: number; text?: string; clear?: boolean; serial?: string
        }
        const s = hub.serialForSession(sessionIdOf(ctx), serial)
        if (action === 'find') return toJson({ serial: s, ...(await hub.uiFind(s, { target, index }, ctx.signal)) })
        if (action === 'click') return toJson(await hub.uiTap(s, { target, index }, ctx.signal))
        if (!text) throw new Error('setText 需要 text 参数')
  ...
  tools.register(
    defineTool({
      ...
      description:
        'Start or stop an mp4 screen recording (adb screenrecord, max 180s per clip) on the claimed device; stop returns the saved file path.',
      parameters: {
      ...
      output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
      execute: async (args, ctx: ToolCtx) => {
        const { serial, action, dir } = args as { serial?: string; action: 'start' | 'stop'; dir?: string }
        const s = hub.serialForSession(sessionIdOf(ctx), serial)
  ...
  tools.register(
    defineTool({
      ...
      description:
        'One-time setup for Chinese text input: install the bundled ADBKeyboard IME on the device (a confirm dialog appears on the phone; MIUI needs "USB install" enabled) and enable it. Call once per device when Chinese text input is required.',
      parameters: { serial: serialParam },
      output: { schema: { type: 'object', additionalProperties: true }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v) }] },
      execute: async (args, ctx: ToolCtx) => {
        const { serial } = args as { serial?: string }
        const s = serial ?? hub.firstOnlineSerial()
        if (!s) throw new Error('没有在线设备')
  ...
  tools.register(
    defineTool({
      ...
      description:
        'Managed debug session — the HOST (not you) enforces screen recording (segmented mp4), full logcat capture and a structured action timeline; you cannot forget or fake it. ' +
      ...
      execute: async (args, ctx: ToolCtx) => {
        const { action, serial, id } = args as { action: 'start' | 'stop' | 'list' | 'analyze'; serial?: string; id?: string }
        if (action === 'list') return toJson({ sessions: hub.debugList() })
        if (action === 'analyze') return toJson(await hub.debugAnalyze(id))
        const s = hub.serialForSession(sessionIdOf(ctx), serial)
        if (action === 'start') return toJson({ serial: s, ...hub.debugStart(s, 'agent', ctx.signal) })
