...
export function apply(ctx: HostCtx, config: Partial<Config> = {}): void {
  ...
  ctx.inject(['webServer', 'tools'], (hostCtx) => {
    ...
    if (host.webServer == null) throw new Error('dsh-android-pane 需要 webServer 服务（web profile 才提供）')
    if (host.tools == null) throw new Error('dsh-android-pane 需要 tools 服务')
    ...
    const unregisterRoutes = mountPaneRoutes(host.webServer as never, hub, (req) => {
 ...
 */
function guardToolUnregister(_host: HostCtx): () => void {
  return () => undefined
}
...
function joinAndroidEmulator(): string | undefined {
