...
import type { ActRequest } from './pane-service.js'
...
function readJsonBody(req: IncomingMessage): Promise<JsonBody | null> {
    ...
    req.on('end', () => {
      ...
      try {
        ...
        if (text.trim() === '') resolve({})
          ...
          else resolve(parsed as JsonBody)
        }
      } catch {
        resolve(null)
      }
...
function strArg(body: JsonBody, key: string): string | undefined {
  const v = body[key]
  return typeof v === 'string' ? v : undefined
}

function numArg(body: JsonBody, key: string): number | undefined {
  const v = body[key]
  return typeof v === 'number' && Number.isFinite(v) ? v : undefined
}

export function mountPaneRoutes(
  ...
  hub: PaneHub,
  sessionIdOf: (req: IncomingMessage) => string | null,
): () => void {
  const unregister = webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const body = method === 'POST' ? await readJsonBody(req) : ({} as JsonBody)
        ...
        if (sub === '/act' && method === 'POST') {
          ...
          const reqAct: ActRequest = {
            action: action as ActRequest['action'],
            serial,
            x: numArg(body ?? {}, 'x'),
            y: numArg(body ?? {}, 'y'),
            x2: numArg(body ?? {}, 'x2'),
        ...
        if (sub === '/stream' && method === 'GET') {
          ...
          const off = session.onFrame((frame) => {
            ...
            if (frame.config) ptsFlags |= 0x8000000000000000n
            if (frame.key) ptsFlags |= 0x4000000000000000n
          ...
          for (const f of session.replayGop()) {
            ...
            let ptsFlags = f.pts
            if (f.config) ptsFlags |= 0x8000000000000000n
            if (f.key) ptsFlags |= 0x4000000000000000n
          ...
          const ping = setInterval(() => {
            session.touchViewer()
            if (!res.writableEnded) res.write(pingFrame)
          }, 5_000)
          const t0 = Date.now()
        ...
        if (!res.headersSent) {
          if (code === 'agent-busy') sendErr(res, 409, 'agent-busy', msg)
          else sendErr(res, 500, 'internal', msg)
        } else {
          try {
            res.end()
          } catch {
