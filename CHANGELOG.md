...
### Fixed
- **B24 样式自愈（头部按钮/面板「破相」修复）**：实测捕获一次瞬时故障——按钮 DOM 在场但 33 条 CSS 全部失效（白底黑框、图标在上文字在下的裸样式），同屏邻插件按钮样式正常，判定为本插件 `<style>` 标签被外力移除/失效后无人恢复（共享 GUI 并行会话热重载竞态为首要嫌疑，触发源无法事后取证）。三层加固：①样式标签纳入 `ctx.effect` 生命周期托管（dispose 先停巡检/断观察器再摘标签，卸载即净）；②内容即版本——apply 时与在册标签逐字比对、不一致原子换新（热重载后新 CSS 即刻生效，免整页刷新）；③自愈链——head MutationObserver（摘除即恢复）+ 5s 巡检兜底 + 按钮挂载逐帧 sentinel（圆角 ≠999px 强制重挂 ≤3 次后 console.warn 点名，fail loud）。视觉与交互零变化。
